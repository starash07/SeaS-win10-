...
> 进度说明（2026-09-15）：本文保留 `V1.0.0` 已实现、已测试的安装基线。当前 `V1.1.0` 沿用本方案，已完成代码保护、安装包封装与自动冒烟测试，并更新了内置 LittleFish 修复构建。
...
当前内置 LittleFish 版本为 `1.5.2`（2026-09-15 修复构建），并包含该构建所需的 `UTF.Unknown 2.7.0` 编码识别依赖。SeaS 只通过以下相对路径启动它：
